...
setupSystem()
{
    ...
    //     goto FAIL;
    // }
    control->shared_ptr = malloc(sizeof(struct shared_data));
    ...
    for (i = 0; i < N_NODES; i++)
    {
        control->shared_ptr->node[i].data_xfer = (char)0;
        control->shared_ptr->node[i].sent = 0;
        control->shared_ptr->node[i].received = 0;
        control->shared_ptr->node[i].terminate = 0;
        memset(&control->shared_ptr->node[i].to_send.data[0], i, sizeof(char) * MAX_DATA);
...
int runSimulation(control, numberOfPackets, ptid)
struct TokenRingData *control;
int numberOfPackets;
pthread_t ptid[N_NODES];
{
    ...
    for (i = 0; i < N_NODES; i++){
        ...
        pthread_create(&ptid[i], NULL, token_node, (void *)data);
    ...
    for (i = 0; i < numberOfPackets; i++)
    {
        ...
        control->shared_ptr->node[num].to_send.length = (rand() % MAX_DATA) + 1;
        // printf("##Sent pkt from %d to %d len %d\n", num, to, control->shared_ptr->node[num].to_send.length);
...
int cleanupSystem(control, ptid)
struct TokenRingData *control;
pthread_t ptid[N_NODES];
{
    ...
    memset(&zeroSemun, '\0', sizeof(union semun));
    /*
