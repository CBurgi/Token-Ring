...
setupSystem()
{
    ...
    for (i = 0; i < N_NODES; i++)
    {
        ...
        memset(&control->shared_ptr->node[i].to_send.data[0], 0, sizeof(char) * MAX_DATA);
        // for(int i=0;i<MAX_DATA;i++){
        //     control->shared_ptr->node[i].to_send.data[i] = i;
        // }
    }
...
int runSimulation(control, numberOfPackets)
struct TokenRingData *control;
int numberOfPackets;
{
    int num, to;
